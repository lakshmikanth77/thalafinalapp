...
def run():
    ...
    if st.button('Predict Group'):
        ...
        st.markdown(f"<h2 style='text-align: center; color: white; background-color: {color};'>Predicted Group: {predicted_group} ({prediction[0]})</h2>", unsafe_allow_html=True)

        st.subheader('Confidence in prediction for each group:')
        confidence_df = pd.DataFrame({
            'Group': [group_dict[i+1] for i in range(len(prediction_proba[0]))],
            'Confidence': prediction_proba[0]
        })
        confidence_df.set_index('Group', inplace=True)
        confidence_df.sort_values('Confidence', ascending=False, inplace=True)
        st.table(confidence_df.style.format({'Confidence': '{:,.2%}'}))
